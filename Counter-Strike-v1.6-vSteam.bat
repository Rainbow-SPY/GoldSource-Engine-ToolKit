...
if exist "%_#APPDATA#%\Log" (
      	echo [%time:~0,2%:%time:~3,2%:%time:~6,2%]==================== Launcher====================>>"%_#APPDATA#%\Log\Log_%date:~0,4%_%date:~5,2%_%date:~8,2%.log"
      	) else (
      	md "%_#APPDATA#%\Log"
      	)
if exist "%_#APPDATA#%" (
...
echo           Support: QQ:2716842407
echo            ***有bug请及时反馈~
echo %_line%
:ChooseCDKEY
mshta vbscript:msgbox("为避免被Valve检测相同CD-KEY，局域网玩家请选择不同CD-KEY进行联机游玩",0+64+4096+65536,"%_ver% Launcher")(window.close)
...
if errorlevel 1 set _Ecode_main= ERROR_INVALID_HANDLE(0x0000003f5) & goto CrashSet
if errorlevel 0 echo Width=640>>%_#APPDATA#%\video.ini
...
if errorlevel 1 set _Ecode_main= ERROR_INVALID_HANDLE(0x0000003f5) & goto CrashSet
if errorlevel 0 echo Width=720>>%_#APPDATA#%\video.ini
goto ChooseScreenWindow
:800x600
...
if errorlevel 1 set _Ecode_main= ERROR_INVALID_HANDLE(0x0000003f5) & goto CrashSet
if errorlevel 0 echo Width=800>>%_#APPDATA#%\video.ini
goto ChooseScreenWindow
:1024x768
...
if errorlevel 1 set _Ecode_main= ERROR_INVALID_HANDLE(0x0000003f5) & goto CrashSet
if errorlevel 0 echo Width=1024>>%_#APPDATA#%\video.ini
goto ChooseScreenWindow
:1152x864
...
if errorlevel 1 set _Ecode_main= ERROR_INVALID_HANDLE(0x0000003f5) & goto CrashSet
if errorlevel 0 echo Width=1152>>%_#APPDATA#%\video.ini
goto ChooseScreenWindow
:1280x960
...
if errorlevel 1 set _Ecode_main= ERROR_INVALID_HANDLE(0x0000003f5) & goto CrashSet
if errorlevel 0 echo Width=1280>>%_#APPDATA#%\video.ini
goto ChooseScreenWindow
:1280x1024
...
if errorlevel 1 set _Ecode_main= ERROR_INVALID_HANDLE(0x0000003f5) & goto CrashSet
if errorlevel 0 echo Width=1440>>%_#APPDATA#%\video.ini
goto ChooseScreenWindow
:1600x900
reg add HKEY_CURRENT_USER\Software\Valve\Half-Life\Settings /v ScreenHeight /t REG_DWORD /f /d "900" >NUL 2>NUL
if errorlevel 1 set _Ecode_main= ERROR_INVALID_HANDLE(0x0000003f5) & goto CrashSet
...
if errorlevel 1 set _Ecode_main= ERROR_INVALID_HANDLE(0x0000003f5) & goto CrashSet
if errorlevel 0 echo Width=1600>>%_#APPDATA#%\video.ini
goto ChooseScreenWindow
:1682x1050
...
if errorlevel 1 set _Ecode_main= ERROR_INVALID_HANDLE(0x0000003f5) & goto CrashSet
if errorlevel 0 echo Width=1682>>%_#APPDATA#%\video.ini
goto ChooseScreenWindow
:1920x1080
...
if errorlevel 1 set _Ecode_main= ERROR_INVALID_HANDLE(0x0000003f5) & goto CrashSet
if errorlevel 0 echo Width=1920>>%_#APPDATA#%\video.ini
goto ChooseScreenWindow
:ScreenWindow
...
:Crashhalflifeexe
echo Not found file!
%windir%\Explorer.exe %_#APPDATA#%\Support & 7za.exe x .\cstrike\resource\Issues.res -o%_#APPDATA#%\Support -aoa >NUL >NUL
mshta vbscript:msgbox("你好像没有把Mod包放进游戏目录里？！",0+64+4096+65536,"%_ver% Launcher - 应用程序错误")(window.close)
exit
:CrashCer
%windir%\Explorer.exe %_#APPDATA#%\Support & 7za.exe x .\cstrike\resource\Issues.res -o%_#APPDATA#%\Support -aoa >NUL
mshta vbscript:msgbox(Replace("应用程序发生异常 (%_Ecode_main%).\n证书安装失败，无法验证证书的完整性。\n\n请联系程序作者并解决问题 ,  要终止程序，请单击[确定]。","\n",vbCrLf),0+16+4096+65536,"%_ver% Launcher - 应用程序错误")(window.close)
exit
:CrashSet
%windir%\Explorer.exe %_#APPDATA#%\Support & 7za.exe x .\cstrike\resource\Issues.res -o%_#APPDATA#%\Support -aoa >NUL
mshta vbscript:msgbox(Replace("应用程序发生异常 (%_Ecode_main%).\n未知错误，写入注册表无效。\n\n请联系程序作者并解决问题 ,  要终止程序，请单击[确定]。","\n",vbCrLf),0+16+4096+65536,"%_ver% Launcher - 应用程序错误")(window.close)
